...
out <- map(1:nrow(ncv), function(i) {
  ...
  rs_i <- sculpt_rough(
    ...
    model_predict_fun = function(x) {
      p <- predict(mod_i, new_data = x, type = "prob")$.pred_1
      logit(p)
    },
    n_ice = 10, 
    seed = 142324
  ...
  # obs_i <- as.numeric(as.character(te_i[[dd$response]]))
  obs_fct_i <- te_i[[dd$response]]
  pred_rs_i <- inv.logit(predict(rs_i, te_i))
  pred_ds_i <- inv.logit(predict(ds_i, te_i))
  pred_ps_i <- inv.logit(predict(ps_i, te_i))
  cm_rs_i <- fit_calibration(obs = obs_fct_i, preds = pred_rs_i)
  cm_ds_i <- fit_calibration(obs = obs_fct_i, preds = pred_ds_i)
  cm_ps_i <- fit_calibration(obs = obs_fct_i, preds = pred_ps_i)
  out[[i]] <- tibble(
    i = i,
    id = ncv$id[[i]],
    id2 = ncv$id2[[i]],
